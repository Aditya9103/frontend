...
import { sendSignupOtp, verifySignupOtp, resendOtp, googleAuth } from '../Redux/Slices/AuthSlice';
...
function Signup() {
    ...
    async function handleSendOtp(event) {
        ...
        const response = await dispatch(sendSignupOtp(formData));
    ...
    async function handleVerifyOtp(e) {
        ...
        setIsSubmitting(true);
        const response = await dispatch(verifySignupOtp({ email: signupData.email, otp: enteredOtp }));
        setIsSubmitting(false);
                    ...
                    {!isOtpSent ? (
                        <form noValidate onSubmit={handleSendOtp} className='flex flex-col gap-5 text-white'>
                            <div className="text-center mb-4">
                                ...
                                <p className="text-gray-300 text-sm mt-2">Sign up to get started</p>
                            </div>

                            <label htmlFor="image_uploads" className="cursor-pointer mx-auto transition-transform hover:scale-105">
                                {previewImage ? (
                                    <img className="w-24 h-24 rounded-full object-cover border-2 border-yellow-500 shadow-lg" src={previewImage} alt="Profile" />
                                ) : (
                            ...
                            <button type="submit" disabled={isSubmitting} className='mt-4 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-400 hover:to-yellow-500 disabled:opacity-50 transition-all rounded-lg py-3 font-bold text-lg text-gray-900 shadow-lg cursor-pointer'>
                            ...
                            <p className="text-center text-sm text-gray-300 mt-2">
                                Already have an account? <Link to="/login" className='text-yellow-500 hover:text-yellow-400 font-semibold underline-offset-4 hover:underline transition-all'> Login</Link>
                            </p>
                        </form>
                    ) : (
                        <div className="flex flex-col items-center text-white">
                            ...
                            <p className="text-gray-300 text-sm text-center mb-6">
                                We've sent a 6-digit code to <span className="font-semibold text-yellow-500">{signupData.email}</span>
                            </p>
